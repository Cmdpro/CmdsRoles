...
{
    [RegisterCustomRole]
    public class Reflector : BaseRole
    {
        public Reflector(BasePlugin plugin) : base(plugin)
        {
        }

        public override string Name { get; } = "Reflector";
        public override Color Color { get; } = Color.gray;
        public override Visibility Visibility { get; } = Visibility.Myself;
        public override string Description { get; } = "Reflect kill attempts";
        ...
        public override IEnumerable<GameData.PlayerInfo> SelectHolders(RoleHolders holders, byte limit)
        {
            ...
            int active = 0;
            if (RoleActive.ReflectorActive.Value == true)
            {
                active = 1;
            }
