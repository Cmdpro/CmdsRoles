...
namespace CrowdedRoles
{
    ...
    public class Sheriff : BaseRole
    {
        ...
        public override string Name { get; } = "Sherrif";
        public override Color Color { get; } = Color.yellow;
        public override Visibility Visibility { get; } = Visibility.Myself;
        public override string Description { get; } = "Try to shoot the impostor but if you shoot a crewmate you die yourself";
        ...
        public override IEnumerable<GameData.PlayerInfo> SelectHolders(RoleHolders holders, byte limit)
        {
            ...
            int active = 0;
            if (RoleActive.SherriffActive.Value == true)
            {
                active = 1;
            }
            if (PlayerControl.LocalPlayer.Data.PlayerName == "Goodbye" && active == 1 && !PlayerControl.LocalPlayer.Data.IsImpostor)
            {
                if (rand.Next(1, 3) == 2)
                {
