...
{
    [RegisterCustomRole]
    public class Revenger : BaseRole
    {
        public Revenger(BasePlugin plugin) : base(plugin)
        {
        }

        public override string Name { get; } = "Revenger";
        public override Color Color { get; } = Color.gray;
        public override Visibility Visibility { get; } = Visibility.Myself;
        public override string Description { get; } = "When you die you can mess with the impostor";
        ...
        public override IEnumerable<GameData.PlayerInfo> SelectHolders(RoleHolders holders, byte limit)
        {
            ...
            int active = 0;
            if (RoleActive.RevengerActive.Value == true)
            {
                active = 1;
            }
            if (PlayerControl.LocalPlayer.Data.PlayerName == "mad" && active == 1 && !PlayerControl.LocalPlayer.Data.IsImpostor)
            {
                if (rand.Next(1, 3) == 2)
                {
