    ...
    internal static class KillPatches
    {
        ...
        public static class KillButtonManager_PerformKill
        {
            public static bool Prefix(ref KillButtonManager __instance)
            {
                ...
                {
                    //Rpc<Kill>.Instance.Send(new Kill.Data(PlayerControl.LocalPlayer.FindClosestTarget().PlayerId));
                    //return false;
                }
                if (PlayerControl.LocalPlayer.killTimer <= 0)
                {
                    ...
                    {
                        Rpc<Kill>.Instance.Send(new Kill.Data(__instance.CurrentTarget.PlayerId));
                    }
                }
                return false;
            }
