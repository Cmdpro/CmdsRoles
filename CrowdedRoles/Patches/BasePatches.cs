...
namespace CrowdedRoles.Patches
{
    internal static class BasePatches
    {
        ...
        public static class IntroCutScene_CoBegin
        {
            ...
            {
                if (RoleSettings.DetHelpKnow.Value)
                {
                    if (PlayerControl.LocalPlayer.Is<Detective>() || PlayerControl.LocalPlayer.Is<Helper>())
                    {
                        foreach (GameData.PlayerInfo i in GameData.Instance.AllPlayers)
                        {
                            if (i.Object.Is<Helper>() || i.Object.Is<Detective>())
                            {
                                i.Object.nameText.color = Color.blue;
                            }
                        }
                    }
                }
